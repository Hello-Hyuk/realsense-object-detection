...
def add_camera_args(parser):
    ...
    parser.add_argument('--rs', type=bool, default=False,
                        help='use realsense device')
...
class Camera():
    ...
    def listener(self,realsense_img):
        # Start node
        rospy.loginfo('Image topic: {}'.format(realsense_img))

        # Subscribe to topics
        rospy.Subscriber(realsense_img,Image,self.callback)
    ...
    def _open(self):
        ...
        if a.image:
            ...
            self._start()
        ######## working out ############
        elif a.rs is not None:
            realsense_img = '/camera/color/image_raw'
            # Start subscriber
            self.listener(realsense_img)
            self._start()
    ...
    def _start(self):
        ...
        if self.video_file:
            ...
            self.img_height, self.img_width, _ = self.img_handle.shape
            print("img_handle else height and width")
